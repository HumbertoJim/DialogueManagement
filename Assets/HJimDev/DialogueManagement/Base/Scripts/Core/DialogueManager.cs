    ...
    namespace Base
    {
        namespace Core
        {
            public class DialogueManager : MonoBehaviour
            {
                ...
                [SerializeField] Modules.Module[] modules;
                ...
                Entities.Dialogue currentDialogue;
                ...
                public TextManager TextManager { get { return textManager; } }
                ...
                public Entities.Dialogue InstantiateDialogue(string dialogueID, int startAt = 0)
                {
                    string[] content = ReadDialogueContent(dialogueID);
                    Entities.Dialogue dialogue = new(dialogueID, content, startAt);
                    return dialogue;
                }
                ...
                {
                    Entities.Dialogue dialogue = InstantiateDialogue(dialogueID);
                    PlayDialogue(dialogue);
                }


                public void PlayDialogue(Entities.Dialogue dialogue)
                {
                ...
                public void Next()
                {
                    ...
                    Entities.Dialogue.Line line = null;
                ...
                bool ExecuteLine(string line)
                {
                    ...
                    else if (line[^1] == ':')
                    {
                        ...
                        {
                            currentDialoguer = currentDialogue.CurrentDialoguer;
                            dialoguerCluster.SetTalker(currentDialoguer);
                    ...
                    else if (Tools.StringExtensions.TextStartsWith(line, keyWords.Command))
                    {
                        ...
                        foreach (Modules.Module module in modules)
                        {
                ...
                void ApplyIf(string line)
                {
                    ...
                    if (!flag)
                    {
                        Entities.Dialogue.Line ignore_line = currentDialogue.Next(ignoreDialoguer: true);
                        int nEndIf = 0;
                    ...
                    for (int i = 0; i < (originalText.Length - n_ini - n_fin); i++)
                    {
                        if (originalText.Substring(i, n_ini) == keyWords.ReplaceIni)
                        {
                            ...
                            for (; i < (originalText.Length - n_fin + 1); i++)
                            {
                                ...
                                {
                                    if (stringID == dialoguerCluster.MainCharacter.Code)
                                    {
